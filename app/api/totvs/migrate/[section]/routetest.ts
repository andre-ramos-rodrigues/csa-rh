...
export async function POST(
  ...
  try {
    ...
    const isFormacaoSection = ['formacaoacademica', 'formacao_academica', 'academics', 'formacao', 'formacaoAcademica', 'formacao-academica'].includes(normalizedSection);
      ...
      if (!CODPESSOA) {
        ...
        try {
          ...
          const rowsAffected = await insertCurso({
            ...
            codCurso: payload?.codCurso ? Number(payload.codCurso) : (payload?.CODCURSO ? Number(payload.CODCURSO) : null),
            codGrau: payload?.codGrau ?? payload?.CODGRAU ?? payload?.grauInstrucao ?? null,
            outroCurso: cleanValue(payload?.outroCurso ?? payload?.OUTROCURSO),
            nomeEntidade: cleanValue(payload?.nomeEntidade ?? payload?.NOMEENTIDADE),
            codEntidade: payload?.codEntidade ? Number(payload.codEntidade) : (payload?.CODENTIDADE ? Number(payload.CODENTIDADE) : null),
            dtInicio: parseDateToIso(payload?.dtInicio ?? payload?.DTINICIO),
            dtTermino: parseDateToIso(payload?.dtTermino ?? payload?.DTTERMINO),
            mesInicio: payload?.mesInicio ? Number(payload.mesInicio) : (payload?.MESINICIO ? Number(payload.MESINICIO) : null),
            anoInicio: payload?.anoInicio ? Number(payload.anoInicio) : (payload?.ANOINICIO ? Number(payload.ANOINICIO) : null),
            mesTermino: payload?.mesTermino ? Number(payload.mesTermino) : (payload?.MESTERMINO ? Number(payload.MESTERMINO) : null),
            anoTermino: payload?.anoTermino ? Number(payload.anoTermino) : (payload?.ANOTERMINO ? Number(payload.ANOTERMINO) : null),
            andamento: payload?.andamento ?? payload?.ANDAMENTO ?? 'C',
            podeComprovar: payload?.podeComprovar ?? payload?.PODECOMPROVAR ?? null,
            infAdic: cleanValue(payload?.infAdic ?? payload?.INFADIC),
            username: currentUsername,
          });
